...
let [coreCss = '', taskCss = ''] = await Promise.all([
    fetchText(paths.coreCss),
    fetchText(paths.taskCss)
])
taskCss = await minifyCss(taskCss)
...
const codesMap: Record<string, string> = Object.fromEntries(codes)
window.codesMap = codesMap
const result = await buildCode('@core/coreScript', true, codesMap)
